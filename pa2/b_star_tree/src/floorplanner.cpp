...
void Floorplanner::parseBlockFile(istream& blockFile)
{
    ...
    cout << "outlineWidth: " << _outlineWidth << ", outlineHeight: " << _outlineHeight << endl;
...
double Floorplanner::computeCost(double alpha, double beta,
    ...
    double outlineXPenaltyWeight = 10;
    double outlineYPenaltyWeight = 10;
    // double cost = alpha * normA + (1-alpha) * normW + beta * normR + 
    //              outlineXPenaltyWeight * normX + outlineYPenaltyWeight * normY;

    double cost = 5 * normA +  10 * normR + outlineXPenalty * normX + outlineYPenalty * normY;
    // Log the metrics
...
void Floorplanner::simulatedAnnealing() {
    ...
    // double T = 100000.0; // original
    double T = 100000.0;

    const double T_min = 0.001; // original is 0.1
    const double cooling_rate = 0.99;
    ...
    *_tree = bestSolution;
    ...
    // second stage: polish the solution
    // newly added ////////////////////////
    double best = bestCost;
    int polishIters = 1000000;
    for (int i = 0; i < polishIters; ++i) {
        Tree backup = *_tree;
        double r = static_cast<double>(rand()) / RAND_MAX;
        if (r < 0.15) {
            _tree->rotateRandom();
        ...
        _tree->pack();
        double c = computeCost(_alpha, _beta, areaNorm, wireNorm, aspectNorm, outlineXNorm, outlineYNorm);
        if (c < best) { best = c; bestSolution = *_tree; }
        else { *_tree = backup; }
    }
    *_tree = bestSolution;
    // newly added ////////////////////////
