...
Solver::Solver() : env_(std::make_unique<GRBEnv>(true)), model_(nullptr), status_(GRB_LOADED) {
  try {
    ...
    std::cerr << "[Gurobi Init Failed] code=" << e.getErrorCode()
              << " msg=" << e.getMessage() << "\n";
    throw; 
  }
}
...
{
    if(s == "MIN") return GRB_MINIMIZE;
    else if(s == "MAX") return GRB_MAXIMIZE;
    else {
...
void Solver::addConstraint(const std::string &name,
...
void Solver::setObjective(const std::vector<std::pair<std::string, double>> &vars,
                                char sense) {
    GRBLinExpr expr = 0.0;
    for (const auto& [vname, coef] : vars) {
        auto it = varmap_.find(vname);
