...
/*
  Build:
    g++ -O3 -march=native -std=c++17 refiner_pixel_even.cpp -o refiner_stage2

  Run:
    ./refiner_stage2 <input_problem.txt> <stage1.out> <final.out>
*/
...
struct Module
{
    ...
    // streak penalty for bbox-extending side
    // 0:left 1:right 2:down 3:up
    int sideStreak[4] = {0, 0, 0, 0};
};

class RefinerPixelEven
{
public:
    int chipW = 0, chipH = 0;
    vector<Module> mods;
    ...
    // grid owner: -1 empty else module id
    vector<int> grid;
    ...
    // ---- tunables ----
    int maxRounds = 500;
    int maxStepsPerModPerRound = 500;
    ...
    double hpwlEps = 0.0; // allow small worsening if you want

    // scoring weights (key for "evenness")
    double dirBias = 1e-3;   // tiny, keep
    double wNeighbor = 80.0; // smoothness bonus (bigger => less spiky)
    double wSpread = 0.02;   // spread bonus per Manhattan distance
    double wStreak = 200.0;  // penalty per consecutive bbox-extension on same side

    // selection randomness (helps avoid deterministic spikes)
    int pickTopK = 8; // pick randomly among top K scored candidates (K=1 => deterministic)
    uint64_t rngSeed = 1234567;
    ...
    bool bboxLegal(const Module &m, int nminx, int nminy, int nmaxx, int nmaxy, long long narea) const
    {
        if (nminx < 0 || nminy < 0 || nmaxx > chipW || nmaxy > chipH)
            return false;
        int w = nmaxx - nminx, h = nmaxy - nminy;
        if (w <= 0 || h <= 0)
            return false;
    ...
    void buildGridAndFrontiers()
    {
        ...
        auto paintRect = [&](int id, int minx, int miny, int maxx, int maxy)
        {
            for (int y = miny; y < maxy; y++)
            {
                ...
                for (int x = minx; x < maxx; x++)
                {
                    int &c = grid[base + (size_t)x];
                    if (c != -1)
                        throw runtime_error("Overlap painting " + mods[id].name + " with " + mods[c].name);
                    c = id;
                }
            }
        };
        ...
        for (auto &m : mods)
        {
            ...
            m.frontier.clear();
            m.hasLast = false;
            m.sideStreak[0] = m.sideStreak[1] = m.sideStreak[2] = m.sideStreak[3] = 0;
            addFrontierFromBBoxBoundary(m);
        }
    }
    ...
    // must be adjacent + legal if added
    bool canAddPixel(const Module &m, int x, int y) const
    {
        if (x < 0 || y < 0 || x >= chipW || y >= chipH)
            return false;
    ...
    int neighborCount4(const Module &m, int x, int y) const
    {
        int cnt = 0;
        if (x > 0 && grid[packCell(x - 1, y, chipW)] == m.id)
            cnt++;
        if (x + 1 < chipW && grid[packCell(x + 1, y, chipW)] == m.id)
            cnt++;
        if (y > 0 && grid[packCell(x, y - 1, chipW)] == m.id)
            cnt++;
        if (y + 1 < chipH && grid[packCell(x, y + 1, chipW)] == m.id)
            cnt++;
        return cnt;
    }
    ...
    void applyAddPixel(Module &m, int x, int y)
    {
        ...
        // update streaks depending on which side bbox extends
        bool extL = (x < m.minx);
        bool extR = (x + 1 > m.maxx);
        bool extD = (y < m.miny);
        bool extU = (y + 1 > m.maxy);

        // default: decay streaks a bit when not extending bbox
        if (!(extL || extR || extD || extU))
        {
            for (int i = 0; i < 4; i++)
                m.sideStreak[i] = max(0, m.sideStreak[i] - 1);
        }
        else
        {
            // if extends multiple sides (corner), treat both
            int used[4] = {extL, extR, extD, extU};
            for (int i = 0; i < 4; i++)
            {
                if (used[i])
                    m.sideStreak[i] += 1;
    ...
    // pick one pixel to add (EVEN growth scoring)
    bool expandOneStep(Module &m)
    {
        if (m.frontier.empty())
            return false;
        ...
        vector<Cand> best;
        best.reserve((size_t)pickTopK);

        auto tryInsert = [&](const Cand &c)
        {
            if ((int)best.size() < pickTopK)
            {
                best.push_back(c);
                return;
            }
            // keep topK by score
            int worst = 0;
            for (int i = 1; i < pickTopK; i++)
                if (best[i].score < best[worst].score)
                    worst = i;
            if (c.score > best[worst].score)
                best[worst] = c;
        };

        // scan frontier bag (lazy cleanup)
        for (int i = (int)m.frontier.size() - 1; i >= 0; i--)
        {
            int p = m.frontier[i];
            if (!inFrontier[m.id][p])
            ...
            // direction dot (weak)
            double cx = centerX(m), cy = centerY(m);
            double vpx = (x + 0.5) - cx, vpy = (y + 0.5) - cy;
            double vmag = hypot(vpx, vpy);
            double dirDot = 0.0;
            if (vmag > 1e-12)
                dirDot = (vpx / vmag) * dx + (vpy / vmag) * dy;

            // smoothness: prefer pixels that attach by 2+ neighbors
            int neigh = neighborCount4(m, x, y);

            // spread: prefer farther from last added pixel
            int dist = 0;
            if (m.hasLast)
                dist = abs(x - m.lastX) + abs(y - m.lastY);

            // streak penalty: discourage extending bbox repeatedly on same side
            double streakPenalty = 0.0;
            if (x < m.minx)
                streakPenalty += wStreak * m.sideStreak[0];
            if (x + 1 > m.maxx)
                streakPenalty += wStreak * m.sideStreak[1];
            if (y < m.miny)
                streakPenalty += wStreak * m.sideStreak[2];
            if (y + 1 > m.maxy)
                streakPenalty += wStreak * m.sideStreak[3];

            // FINAL score
            // main term: -dHP (want HPWL decrease)
            // evenness terms: neighbor bonus + spread bonus - streak penalty
            double score =
                (-dHP) + dirBias * dirDot + wNeighbor * (double)neigh + wSpread * (double)dist - streakPenalty;

            tryInsert({score, dHP, x, y});
        }

        if (best.empty())
            return false;

        // pick among topK randomly proportional to positive-shifted score
        double minS = best[0].score, maxS = best[0].score;
        for (auto &c : best)
        {
            minS = min(minS, c.score);
            maxS = max(maxS, c.score);
        }
        // shift so all weights >= small positive
        double shift = (minS < 0.0) ? (-minS + 1e-9) : 1e-9;

        double totalW = 0.0;
        vector<double> w(best.size());
        for (size_t i = 0; i < best.size(); i++)
        {
            w[i] = best[i].score + shift;
            totalW += w[i];
        }
        uint64_t r = xorshift64();
        double u = (double)(r % 1000000ULL) / 1000000.0;
        double t = u * totalW;
        ...
        // acceptance gate
        double dHP = best[pick].dHP;
        if (dHP < 0.0 || (allowNeutral && dHP == 0.0) || dHP <= hpwlEps)
        {
            applyAddPixel(m, best[pick].x, best[pick].y);
            return true;
        }
        return false;
    }
    ...
    vector<pair<int, int>> extractPolygon(const Module &m) const
    {
        ...
        for (int y = m.miny; y < m.maxy; y++)
        {
            for (int x = m.minx; x < m.maxx; x++)
            {
                if (!isOcc(x, y))
                    continue;
                if (!isOcc(x - 1, y))
                    edges.push_back({x, y, x, y + 1, 1});
                if (!isOcc(x + 1, y))
                    edges.push_back({x + 1, y + 1, x + 1, y, 3});
                if (!isOcc(x, y - 1))
                    edges.push_back({x + 1, y, x, y, 2});
                if (!isOcc(x, y + 1))
                    edges.push_back({x, y + 1, x + 1, y + 1, 0});
            }
        }
        if (edges.empty())
        {
            return {{m.minx, m.miny}, {m.minx, m.maxy}, {m.maxx, m.maxy}, {m.maxx, m.miny}};
        }

        unordered_map<long long, vector<int>> out;
        out.reserve(edges.size() * 2);

        long long startLex = LLONG_MAX;
        int startEdge = -1;
        for (int i = 0; i < (int)edges.size(); i++)
        {
            auto &e = edges[i];
            out[keyVertex(e.x1, e.y1)].push_back(i);
            long long lex = ((long long)e.y1 << 32) ^ (unsigned int)e.x1;
            if (lex < startLex)
            {
                startLex = lex;
        ...
        while (true)
        {
            ...
            auto it = out.find(keyVertex(cx, cy));
            if (it == out.end())
                break;
...
int main(int argc, char **argv)
{
    ...
    if (argc < 4)
    {
        cerr << "Usage: ./refiner_stage2 <input> <stage1_out> <final_out>\n";
        return 1;
    }
