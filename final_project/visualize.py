...
def plot_result(chip_w, chip_h, fixed_modules, soft_modules,
                connections, hpwl, output_img="result.png",
    ...
    # 4. Draw Connections (optional)
    
    if plot_connections and connections:
        module_centers = get_module_centers(fixed_modules, soft_modules)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-conn', action='store_true', help='Do not plot connections')
    ...
    # Plot
    plot_result(
        cw, ch, fixed, soft, connections, hpwl,
        args.plot_file,
        plot_connections=not args.no_conn
    )
